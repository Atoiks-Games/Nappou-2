...
public final class StreamBeamPattern extends TimedCounter {

    private static final Vector2 ACCEL = new Vector2(0, 10000);

    public StreamBeamPattern() {
    ...
    public void onTimerUpdate(Enemy enemy, float dt) {
        switch (this.getCount() % 300) {
            ...
            case 50: {
                ...
                final Vector2 velocity = new Vector2(game.player.getX(), Math.min(game.player.getY() - 200, -15))
                        .sub(pos)
                        .normalize()
                        .mul(2000);
